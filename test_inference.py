...
def run_inference_on_combinations(image_folder, audio_folder):
    # Define the extensions for images and audios you want to consider
    image_extensions = ('.jpg', '.jpeg')
    audio_extensions = ('.wav')
    
    # Get all the image and audio files
    image_files = get_files_from_directory(image_folder, image_extensions)
    audio_files = get_files_from_directory(audio_folder, audio_extensions)
    ...
    # Loop through each combination of image and audio
    for image_path in image_files:
        for audio_path in audio_files:
            
            # Assuming the output is named by combining the input names
            # (Replace with actual naming convention)
            output_filename = predict_output_name(image_path, audio_path)
            ...
            logging.info(f"Processing combination {combination_index}/{total_combinations}: image {image_path} and audio {audio_path}")
            ...
            cmd = [
                ...
                "--output_dir", "G:/My Drive/5GS/batch_180924_ryan_z115_yn1_test",
                "--cross_id",
                "--if_extract",
                "--if_align",
            ]
...
def predict_output_name(image_path, audio_path):
    # Extract base names without extension
    image_name = os.path.splitext(os.path.basename(image_path))[0]
    audio_name = os.path.splitext(os.path.basename(audio_path))[0]

    # Combine them to produce the output name
    return f"{image_name}_{audio_name}.mp4"

if __name__ == "__main__":
    image_folder = "batch_180923_resized_solo"
    audio_folder = "audio"
    run_inference_on_combinations(image_folder, audio_folder)
